...
def valid_space(shape, grid, IA):  # saver si esta la fitxa està en un lloc valid basicament mirant
    ...
    for pos in formatted:
        if pos not in accepted_pos:
            if pos[
                1] > -1 or IA != 0:  # he agut de posar IA!=0 perquè si no em detectava posicions que no eren valides perque la IA=1 anava més cap amunt de y=-1 llavors detectava posició valida, en canvi a la IA=0 és necesari perquè quan la peça llarga apareix detecta que està mal colocada al principi
                return False
    return True
...
def comprovar_forats(win1, grid, cas):  # comprovar els forats que hi ha tenint en compte
    ...
    for j in range(len(grid[
                           1])):  # veure si hi ha forats a partir dels canvis de espai buit a espai pintats (comprova de dalt cap abaix i d'equerra a dreta)
        no = 0
        si = 0
        dis = 0
...
def main_menu(win):
    ...
    while run:
        ...
        pygame.display.update()
        if dada=="1" or dada=="2" or dada=="3":#no he ficat que no sigui igual a 0 pels strings
